...
/// Scriptable in-memory backend that powers the cn1lib's JavaSE port.
///
/// Backed by [BluetoothSimulator]: tests and the simulator menu drive its
/// state directly. All bridge methods return true (action accepted for
/// dispatch) and deliver real success/error payloads asynchronously through
/// [BluetoothCallbackRegistry.sendResult] — exactly the contract the iOS and
/// Android implementations honor.
///
/// Selected by [BluetoothNativeBridgeImpl] when
/// `cn1.bluetoothle.javase.backend=simulator` (the default). The
/// [CoreBluetoothBackend] is the alternative for real BLE on macOS.
final class SimulatorBluetoothBackend implements BluetoothNativeBridge {
